...
def IVDM3Seg_transform():

    transform = ts.Compose([ts.ToTensor()])

    return transform


def FetalBrain_transform(sample, train_type):
    image, label = Image.fromarray(np.uint8(sample['image'] * 255), mode='L'), \
                   Image.fromarray(np.uint8(sample['label']), mode='L')

    # print(image.shape, label.shape)
    # print(np.uint8(sample['label']).max(), np.uint8(sample['label']).min())
    if train_type == 'train':
        # image, label = randomcrop(size=196)(image, label)
        image, label = randomflip_rotate(image, label, p=0.5, degrees=30)
    # else:
        # image, label = randomcrop(size=256)(image, label)

    image = ts.Compose([ts.ToTensor(),
                        ts.Normalize(mean=(0.485, 0.456, 0.5), std=(0.225, 0.225, 0.5))])(image)
    # label = ts.ToTensor()(label)
    label = torch.from_numpy(np.asarray(label)).unsqueeze(dim=0)
    # print(label.max(), label.min())

    return {'image': image, 'label': label}


def Lung2dseg_transform(sample, train_type):
    image, label = sample['image'], sample['label']
    image = 255*((image - np.min(image)) / (np.max(image) - np.min(image)))

    image, label = Image.fromarray(np.uint8(image), mode='L'), Image.fromarray(label, mode='L')
...
def random_rotate(img, lab, p=0.5, axes=(0,1), degrees=0):
    if random.random() < p:
        if isinstance(degrees, numbers.Number):
            if degrees < 0:
                raise ValueError("If degrees is a single number, it must be positive.")
            ...
            degrees = degrees
        if len(axes) !=2:
            axes = random.sample(axes, 2)
        else:
            axes = axes
        angle = random.uniform(degrees[0], degrees[1])
        img = ndimage.rotate(img, angle, axes=axes, order=0, reshape=False)
        lab = ndimage.rotate(lab, angle, axes=axes, order=0, reshape=False)
