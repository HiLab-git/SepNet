...
def get_bound_coordinate(file, pad=[0,0,0]):
    ...
    for i in range(len(pad)):
        maxpoint[i] = min(maxpoint[i]+pad[i], file_size[i])
...
def load_nifty_volume_as_array(filename, transpose=True, return_spacing=False):
    """
...
def img_multi_thresh_normalized(file, thresh_lis=[0], norm_lis=[0]):
    """
    :param file: np array
    :param upthresh:
    :param downthresh:
    ...
    :return:
    """
    new_file = np.zeros_like(file).astype(np.float)
...
def resize_Multi_label_to_given_shape(volume, zoom_factor,class_number, order = 3):
    """
    ...
    volum_one_reshape = [ndimage.interpolation.zoom(volume_one[i+1], zoom_factor, order=order) for i in range(class_number-1)]
    output = np.zeros_like(volum_one_reshape[0])
    for i in range(class_number-1):
        output = np.rint(volum_one_reshape[i])*(i+1)+output
    return output
