...
class QuadResSeparateConv3D(nn.Module):
    '''
    在xyz方向上分开卷积，xy方向上卷4次，z方向上2次
    '''
    def __init__(self, cin, cout, norm='in', droprate=0, depth=False, dilat=1, pad=1):
        ...
        self.model = nn.Sequential(OrderedDict([
            ...
            ('norm1_2', Norm(cout)),
            ('relu1_2', nn.ReLU()),
            ('conv1_2_depth_deep', depthwise(cout, cout, conv[1], padding=padding[1], depth=depth, dilation=dilation[1])),
            ('drop1_2_deep', nn.Dropout(droprate)),
            ('norm1_2_deep', Norm(cout)),
            ('relu1_2_deep', nn.ReLU()),
            ('conv1_3_depth', depthwise(cout, cout, conv[0], padding=padding[0], depth=depth, dilation=dilation[0])),
            ('drop1_3', nn.Dropout(droprate)),
            ('norm1_3', Norm(cout)),
            ('relu1_3', nn.ReLU()),
